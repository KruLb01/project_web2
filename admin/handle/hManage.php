    ...
    if(isset($_GET['page'])) {
        ...
        if ($page == 'Manage Permission'){
            ...
            if (isset($_GET['idView'])) {
                $id = $_GET['idView'];
                $idRes = $conn->selectData("SELECT chuc_nang.id_chucnang as i
                                FROM chuc_nang,chitiet_quyen_chucnang 
                ...
                while($line=mysqli_fetch_array($res)) {
                    ...
                    while ($tmpLine=mysqli_fetch_array($temp)) {
                        ...
                        if ((int)$tmpLine['vi_tri']%1000==0) {
                            $show .= " class='dm-pop-up-main'";
                        }
                        $show .= "><input type=checkbox ";

                        // if (mysqli_num_rows($checkbox=$conn->selectData("SELECT * FROM chuc_nang,chitiet_quyen_chucnang 
                        //                                     WHERE chuc_nang.id_chucnang = chitiet_quyen_chucnang.id_chucnang 
                        //                                     AND chitiet_quyen_chucnang.id_quyen = '".$line['id_quyen']."' 
                        //                                     and chuc_nang.id_chucnang = '".$tmpLine['id_chucnang']."'")) != 0) {
                        //     $show .= 'checked';
                        // }
