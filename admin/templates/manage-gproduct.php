<div class="dashboard-manage-container">
    <div class="dashboard-manage-title-bar disable-copy">
        <span id='dm-title'>Manage gProducts</span>
    </div>
    <div class="dashboard-manage-show-bar disable-copy">
        ...
        <div class="dashboard-manage-show-bar-fnc">
            <a id='dm-add-btn'><i class="fas fa-plus-circle"></i> Add gProduct</a>
            ...
            <div class="dashboard-manage-search-bar-filter">
                <span>
                    ...
                    <select name="" id="" style='color:#8aa8f7'>
                        ...
                        <option value="id_dongsanpham">Id</option>
                        <option value="ten_dongsanpham">Name</option>
                        <option value="thuonghieu_sanpham">Brand</option>
                    </select>
                </span>
            </div>
        </div>
    ...
    <div class="dashboard-manage-table">
        ...
        <div class="dashboard-manage-table-pagination disable-copy">
            <div class="dashboard-manage-table-pagination-items">
                ...
                <?php
                    $count = ceil(mysqli_fetch_array($conn->selectData('select count(*) as count from dong_san_pham'))['count']/10);
...
<script>
    ...
    $(document).on("click" , ".dashboard-manage-table-action-items li" , function() {
        if ($(this).text().trim() == 'Details') {
            ...
            $.get('handle/hManage.php', {page:'Manage gProducts',popUp:'true',clickPos:just_click}, function(res){
        ...
        if (e.target==$(this).find('.save-btn').eq(0)[0]) {
            ...
            $.get("handle/hManage.php", {page:"Manage gProducts",update:'true',val:'text~'+val1+'~'+val2+'~'+val3}, function(res) {
                if (res.trim()==true) {
                    ...
                    $.get('handle/hManage.php',{page:"Manage gProducts",num:num,pag:'1'},function(res) {
    ...
    $('.dashboard-manage-pop-up-add-info input').keyup(function(e) {
        ...
        var brand = general.eq(2).val().trim();

        // Check
        if (e.target==general.eq(0)[0]) {
            $.get("handle/validateAdd.php", {page:'gproducts',id:id}, function(res) {
                if (res.trim()=='Error')  {
                    setDisable(0);
                    flagc[0] = false;
                } else {
        ...
        if (e.target==general.eq(1)[0]) {
            $.get("handle/validateAdd.php", {page:'gproducts1',id:name}, function(res) {
                if (res.trim()=='Error')  {
                    setDisable(1);
                    flagc[1] = false;
                } else {
    ...
    $('.dm-pop-up-add-save-btn').click(function(){
        ...
        $.get('handle/hManage.php', {page:'Manage gProducts',add:'true',valText:id+'-'+name+'-'+brand}, function(res) {
            if (res.trim() == true) {
                alert('Add new cproducts successfully !');
                var num = $('#dm-select-show').val();
                var pag = $('.dm-selected').text().trim();
    
                $.get('handle/hManage.php',{page:'Manage gProducts',num:num,pag:pag},function(res) {
                    $('.dashboard-manage-table-items').html(res);
                })

                $.get('handle/hManage.php',{page:'Manage gProducts',num:num,pag:pag,numPag:'true',textShow:'true'},function(res) {
